...
public class GenericSpecification<T> implements Specification<T> {
    ...
    public Predicate toPredicate(Root<T> root, CriteriaQuery<?> query, CriteriaBuilder builder) {
        ...
        switch (operation) {
            case EQUALS:
                return builder.equal(path, castValue(value, path.getJavaType()));

            case NOT_EQUALS:
                return builder.notEqual(path, castValue(value, path.getJavaType()));

            case GREATER_THAN:
                return builder.greaterThan(path.as(Comparable.class), (Comparable) castValue(value, path.getJavaType()));

            case GREATER_THAN_OR_EQUAL:
                return builder.greaterThanOrEqualTo(path.as(Comparable.class), (Comparable) castValue(value, path.getJavaType()));

            case LESS_THAN:
                return builder.lessThan(path.as(Comparable.class), (Comparable) castValue(value, path.getJavaType()));

            case LESS_THAN_OR_EQUAL:
                return builder.lessThanOrEqualTo(path.as(Comparable.class), (Comparable) castValue(value, path.getJavaType()));
            ...
            case NOT_IN:
                if (value instanceof List) {
                    return builder.not(path.in((List<?>) value));
                } else if (value instanceof String) {
                    String[] values = value.toString().split(",");
                    List<Object> list = new ArrayList<>();
                    for (String v : values) {
                    ...
                    return builder.not(path.in(list));
                }
